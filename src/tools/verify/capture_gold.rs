//! Capture gold standard CSVs from MSSQL by running each op via sqlcmd.
//!
//! Direct port of `scripts/capture_gold.py`.
//!
//! This runs INSIDE the codespace via codespacectl. It executes each op
//! against MSSQL via `docker exec sqlcmd` and captures the output as
//! gold-standard CSV files.

use std::path::{Path, PathBuf};
use std::process::{Command, Stdio};
use std::time::Instant;

use super::super::common::csv_norm::md5_of_text;

/// sqlcmd base command — runs inside Docker container `mssql-test`.
const SQLCMD_BASE: &[&str] = &[
    "docker",
    "exec",
    "-i",
    "mssql-test",
    "/opt/mssql-tools18/bin/sqlcmd",
    "-S",
    "localhost",
    "-U",
    "sa",
    "-P",
    "YourStrong@Passw0rd",
    "-C",
    "-l",
    "60",
    "-t",
    "300",
    "-d",
    "MSSQL_Advanced_Demo",
    "-W",  // trim trailing whitespace
    "-s", ",",  // column separator
    "-h", "-1", // no headers
    "-w", "65535", // wide rows
    "-r", "1",  // stderr messages
];

/// Run an op via sqlcmd and return (stdout, stderr, exit_code, elapsed_s).
///
/// Direct port of `run_op()` from `capture_gold.py`.
fn run_op(sql_text: &str) -> (String, String, i32, f64) {
    let start = Instant::now();
    let mut child = match Command::new("docker")
        .args(&SQLCMD_BASE[1..])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    {
        Ok(c) => c,
        Err(e) => return (String::new(), e.to_string(), -1, 0.0),
    };

    use std::io::Write;
    if let Some(mut stdin) = child.stdin.take() {
        let _ = stdin.write_all(sql_text.as_bytes());
    }

    match child.wait_with_output() {
        Ok(output) => {
            let elapsed = start.elapsed().as_secs_f64();
            let stdout = String::from_utf8_lossy(&output.stdout).into_owned();
            let stderr = String::from_utf8_lossy(&output.stderr).into_owned();
            let code = output.status.code().unwrap_or(-1);
            (stdout, stderr, code, elapsed)
        }
        Err(e) => (String::new(), e.to_string(), -1, 0.0),
    }
}

/// Clean sqlcmd output: strip "(N rows affected)" lines and noise.
///
/// Direct port of the cleaning logic in `main()` from `capture_gold.py`.
fn clean_output(stdout: &str) -> Vec<String> {
    let mut cleaned: Vec<String> = Vec::new();
    for line in stdout.lines() {
        if line.starts_with("(0 rows affected)")
            || line.starts_with("(1 rows affected)")
            || line.starts_with("Changed database context")
            || line.starts_with("Msg ")
        {
            continue;
        }
        // Match "(N rows affected)" pattern
        let trimmed = line.trim_start();
        if trimmed.starts_with('(') && trimmed.contains("rows affected)") {
            continue;
        }
        cleaned.push(line.to_string());
    }
    // Remove trailing empty lines
    while cleaned.last().map(|l| l.trim().is_empty()).unwrap_or(false) {
        cleaned.pop();
    }
    cleaned
}

/// Capture gold standard CSVs for all 50 ops.
///
/// Direct port of `main()` from `capture_gold.py`.
pub fn capture_all(ops_dir: &Path, out_dir: &Path) -> anyhow::Result<()> {
    std::fs::create_dir_all(out_dir)?;
    let mut summary: Vec<(u32, String, usize, String, f64, String)> = Vec::new();

    for op_num in 1..=50u32 {
        let op_file = ops_dir.join(format!("op_{:02}.sql", op_num));
        if !op_file.exists() {
            continue;
        }
        let sql_text = std::fs::read_to_string(&op_file)?;
        eprintln!("--- OP {:02} ---", op_num);

        let (stdout, stderr, exit_code, elapsed) = run_op(&sql_text);

        let cleaned = clean_output(&stdout);
        let out_csv = out_dir.join(format!("op_{:02}.csv", op_num));

        let csv_content = if cleaned.is_empty() {
            String::new()
        } else {
            format!("{}\n", cleaned.join("\n"))
        };
        std::fs::write(&out_csv, &csv_content)?;

        let hash = md5_of_text(&csv_content);
        let row_count = cleaned.len();
        let status = if exit_code == 0 && row_count > 0 {
            "OK"
        } else if row_count == 0 {
            "NO_RESULTS"
        } else {
            "FAIL"
        };
        let err_preview: String = stderr.chars().take(200).collect();
        eprintln!(
            "  {}  rows={}  hash={}  elapsed={:.2}s",
            status,
            row_count,
            &hash[..16.min(hash.len())],
            elapsed,
        );
        if !stderr.is_empty() && exit_code != 0 {
            let err_preview: String = stderr.chars().take(300).collect();
            eprintln!("  STDERR: {}", err_preview);
        }
        summary.push((
            op_num,
            status.to_string(),
            row_count,
            hash,
            elapsed,
            err_preview,
        ));
    }

    // Write summary
    let summary_path = out_dir.join("summary.csv");
    use std::io::Write;
    let mut f = std::fs::File::create(&summary_path)?;
    writeln!(f, "op_id,status,row_count,hash,elapsed_s,stderr")?;
    for (op_num, status, rc, h, el, err) in &summary {
        writeln!(f, "{},{},{},{},{:.2},\"{}\"", op_num, status, rc, h, el, err)?;
    }

    let ok = summary.iter().filter(|s| s.1 == "OK").count();
    eprintln!("\n=== SUMMARY: {}/50 OK ===", ok);
    Ok(())
}

/// Configuration for gold capture.
#[derive(Debug, Clone)]
pub struct CaptureConfig {
    pub ops_dir: PathBuf,
    pub out_dir: PathBuf,
}

impl CaptureConfig {
    pub fn new(root: &str) -> Self {
        Self {
            ops_dir: PathBuf::from(root).join("scripts/ops_individual"),
            out_dir: PathBuf::from(root).join("gold_standard"),
        }
    }
}
