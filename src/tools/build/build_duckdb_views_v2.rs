...
//!
//! Direct port of `scripts/build_duckdb_views_v2.py`.
//!
//! Adds CAST(... AS TIMESTAMP) for datetime2 columns stored as VARCHAR.

use std::path::Path;

use super::build_duckdb_views::VIEWS_TO_DROP;
use super::super::common::duckdb_conn::{execute, open_read_write};

/// All view DDL statements (v2 — with CAST for VARCHAR timestamps).
const VIEW_DDLS: &[&str] = &[
...
/// Create all views and macros (v2).
///
/// Direct port of the top-level script in `build_duckdb_views_v2.py`.
pub fn build(db_path: &Path) -> anyhow::Result<()> {
    ...
    eprintln!("All views and macros created (v2)");
    Ok(())
}

const DDL_VW_PRODUCT_SUMMARY: &str = r#"CREATE VIEW Sales.vw_ProductSummary AS
SELECT p.Category, COUNT(*) AS ProductCount, SUM(p.BasePrice) AS TotalBasePrice, SUM(p.CostPrice) AS TotalCostPrice
FROM Sales.Products AS p GROUP BY p.Category"#;

const DDL_VW_ALL_TRANSACTIONS: &str = r#"CREATE VIEW Sales.vw_AllTransactions AS
SELECT TransactionID, EmployeeID, ProductID, Quantity, UnitPrice, DiscountPct, TotalAmount, TransactionDate, Region, TransactionDetails, PaymentStatus
FROM Sales.Transactions
UNION ALL
SELECT TransactionID, NULL::INTEGER AS EmployeeID, ProductID, NULL::INTEGER AS Quantity, NULL::DECIMAL(18,4) AS UnitPrice, NULL::DECIMAL(5,4) AS DiscountPct, CAST(Amount AS DECIMAL(18,2)) AS TotalAmount, CAST(ArchiveDate AS VARCHAR) AS TransactionDate, NULL::VARCHAR AS Region, NULL::VARCHAR AS TransactionDetails, NULL::VARCHAR AS PaymentStatus
...
SELECT t.TransactionDate, COUNT(*) AS TransactionCount, SUM(t.TotalAmount) AS DailyTotal, AVG(t.TotalAmount) AS AvgTransaction, COUNT(DISTINCT t.EmployeeID) AS ActiveEmployees
FROM Sales.Transactions AS t GROUP BY t.TransactionDate"#;

const DDL_FN_GET_EMPLOYEE_SALES: &str = r#"CREATE MACRO Sales.fn_GetEmployeeSales(employee_id, start_date, end_date) AS TABLE
SELECT t.TransactionID, t.TransactionDate, t.ProductID, t.Quantity, t.UnitPrice, t.DiscountPct, t.TotalAmount, t.PaymentStatus
FROM Sales.Transactions AS t
WHERE t.EmployeeID = employee_id AND CAST(t.TransactionDate AS TIMESTAMP) >= start_date AND CAST(t.TransactionDate AS TIMESTAMP) <= end_date"#;

const DDL_VW_EMPLOYEE_QUARTERLY_SALES: &str = r#"CREATE VIEW Sales.vw_EmployeeQuarterlySales AS
SELECT EmployeeID, FullName, SaleYear,
...
FROM (
    ...
    FROM HR.Employees AS e INNER JOIN Sales.Transactions AS t ON e.EmployeeID = t.EmployeeID
) AS SourceTable
...
SELECT EmployeeID, FullName, SaleYear, 'Q1' AS Quarter, Q1 AS Amount FROM Sales.vw_EmployeeQuarterlySales WHERE Q1 IS NOT NULL
UNION ALL
SELECT EmployeeID, FullName, SaleYear, 'Q2' AS Quarter, Q2 AS Amount FROM Sales.vw_EmployeeQuarterlySales WHERE Q2 IS NOT NULL
UNION ALL
SELECT EmployeeID, FullName, SaleYear, 'Q3' AS Quarter, Q3 AS Amount FROM Sales.vw_EmployeeQuarterlySales WHERE Q3 IS NOT NULL
UNION ALL
SELECT EmployeeID, FullName, SaleYear, 'Q4' AS Quarter, Q4 AS Amount FROM Sales.vw_EmployeeQuarterlySales WHERE Q4 IS NOT NULL"#;
...
WITH RECURSIVE Hierarchy AS (
    ...
    FROM HR.Employees AS e INNER JOIN Hierarchy AS h ON e.ManagerID = h.EmployeeID
    WHERE h.Level < 10
)
...
SELECT e.Department AS Department, e.FullName AS Employee,
    ...
    COUNT(*) AS TransactionCount, SUM(t.TotalAmount) AS TotalSales, AVG(t.TotalAmount) AS AvgSales
...
SELECT e.FullName, t.TransactionDate, t.TotalAmount,
    SUM(t.TotalAmount) OVER (PARTITION BY e.FullName ORDER BY CAST(t.TransactionDate AS TIMESTAMP) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS RunningTotal,
    RANK() OVER (PARTITION BY e.FullName ORDER BY t.TotalAmount DESC) AS SalesRank,
    LAG(t.TotalAmount, 1) OVER (PARTITION BY e.FullName ORDER BY CAST(t.TransactionDate AS TIMESTAMP)) AS PrevAmount,
    LEAD(t.TotalAmount, 1) OVER (PARTITION BY e.FullName ORDER BY CAST(t.TransactionDate AS TIMESTAMP)) AS NextAmount
FROM HR.Employees e JOIN Sales.Transactions t ON e.EmployeeID = t.EmployeeID"#;
