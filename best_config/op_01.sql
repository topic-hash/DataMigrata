...
-- Translated from MSSQL T-SQL to DuckDB dialect
-- Rewrite: HIERARCHYID → recursive CTE with materialized path
-- Correctness: MATCH (hash matches MSSQL gold standard)

WITH RECURSIVE EmployeeHierarchy AS (
    SELECT EmployeeID, ManagerID, FullName, Department, Salary, JobTitle,
    ...
    UNION ALL
    SELECT e.EmployeeID, e.ManagerID, e.FullName, e.Department, e.Salary, e.JobTitle,
...
FROM EmployeeHierarchy ORDER BY PathString LIMIT 100
