#!/usr/bin/env python3
"""
Build the canonical DuckDB database with all views, functions, and proper schema.
This is the BASELINE schema (Variant A).
"""
import duckdb
import os
import sys
...
try:
    ...
    print(f"Spatial extension warning: {e}")
...
# Drop existing functions/macros
for fn in ['fn_GetEmployeeSales', 'sp_set_session_context']:
    try:
        con.execute(f"DROP MACRO IF EXISTS Sales.{fn}")
    except:
        pass
try:
    con.execute("DROP MACRO IF EXISTS SESSION_CONTEXT")
except:
    pass
...
# 5. Sales.fn_GetEmployeeSales as a table macro
con.execute("""
...
# 6. Sales.vw_EmployeeQuarterlySales (PIVOT)
con.execute("""
...
# 7. Sales.vw_NormalizedQuarterlySales (UNPIVOT)
con.execute("""
...
# 8. HR.vw_ManagerHierarchy (recursive CTE)
con.execute("""
...
# 9. Sales.vw_MultiDimensionalSales (GROUPING SETS)
con.execute("""
...
# 10. Sales.vw_RunningTotalsAndRanks (window functions)
con.execute("""
...
views = con.execute("""
    ...
    WHERE table_schema IN ('HR', 'Sales')
    ORDER BY 1, 2
""").fetchall()
print(f"\nViews in DuckDB ({len(views)}):")
for v in views:
    print(f"  {v[0]}.{v[1]}")
